...
# charger les données
load("~/Bureau/CHAPT_1/raw_predictors.RData")
med_covariates <- pred_tr

# Load occurrences (sites × species)
occ_pooled_v1.1 <- readr::read_csv("~/Bureau/marieke/occ_pooled_v1.1.csv")
...
load("~/Bureau/CHAPT_1/scale_predictors.RData")
...
ggsave("factor_distributions.png", figure, width = 18, height = 12, dpi = 300)
...
ggsave("sampling_effort_method_region.png", meth_region, width = 12, height = 10, dpi = 300)
...
```{r}
occ_pooled_v1_0 <- readr::read_csv("~/Bureau/marieke/occ_pooled_v1.1.csv")
...
surface_detected <- surface_data %>%
  filter(rowSums(select(., all_of(elasmobranchs))) > 0)
...
surface_data <- surface_data %>%
  mutate(any_species_present = if_else(rowSums(select(., all_of(elasmobranchs))) > 0, 1, 0),
...
occ_long <- occ_with_region %>%
  select(region, all_of(species_cols)) %>%
...
pres_matrix <- occ_with_method %>%
  select(method, all_of(species_cols)) %>%
  pivot_longer(cols = all_of(species_cols),
               names_to = "species", values_to = "value") %>%
  mutate(pres = as.integer(value > 0)) %>%
...
site_matrix <- occ_pooled_v1_0 %>%
  select(replicates, all_of(species_cols)) %>%
  mutate(across(all_of(species_cols), ~ ifelse(. > 0, 1, 0)))

site_matrix <- site_matrix %>%
  left_join(med_covariates %>% st_drop_geometry() %>% select(replicates, method),
            by = "replicates")
...
dist_site <- vegdist(site_matrix %>% select(all_of(species_cols)),
                     method = "jaccard")
...
# Load predictors (if not already loaded)
load("~/Bureau/CHAPT_1/raw_predictors.RData")   # contains pred_tr
med_covariates <- pred_tr                       # rename for clarity

# Load occurrences (sites × species)
occ <- readr::read_csv("~/Bureau/marieke/occ_pooled_v1.1.csv")
...
ggsave("species_accumulation_by_region.png", p_acc,
