...
occ <- read.csv("./data/processed_data/eDNA/occ_pooled_v1.0.csv", sep = ",", header=T)
...
## Chekc NA in traits -----
# Check species in occ that are not in traits
length(setdiff(rownames(adne), traits$Species)) # 24 species not in traits matrix
setdiff(rownames(adne), traits$Species) # Species from species complex + 5 species
# [1] "Acanthurus coeruleus"      "Facciolella oxyrhynchus"  
# [3] "Arctozenus risso"          "Parablennius sp 1"        
# [5] "Parablennius sp 2"         "Tripterygion melanurus"   # Tripterygion melanurum
# [7] "Gaidropsarus sp"           "Coelorinchus caelorhincus"
# [9] "Knipowitschia caucasica"   "Istiophorus platypterus"  
# [11] "Notacanthus chemnitzii"    "Epinephelus costae"       
# [13] "Triglinae sp"              "Chelidonichthys sp"       
# [15] "Triglinae sp 1"            "Salmo salar"              
# [17] "Scomber colias"            "Sparidae sp 2"            
# [19] "Oblada melanurus"  # Oblada melanura        "Stomias boa" # Stomias boa boa              
# [21] "Hyperoplus lanceolatus"    "Alopias superciliosus" # Alopias vulpinus
# [23] "Dipturus oxyrinchus"       "Raja sp"         
...
# select traits
traits_fd <- traits %>%
  dplyr::select(species, # Extracts only the relevant trait columns
                a, 
                b, 
                K, 
                Schooling,
  ...
  distinct(species, .keep_all = TRUE) %>% # Removes any duplicated species rows
  tibble::column_to_rownames(var = "species") %>% # Sets the species names as row names
  mutate_if(sapply(., is.character), as.factor) %>% # Converts character var as factor
  mutate_if(sapply(., is.integer), as.factor) # Converts integer var as factor


# Build the traits category matrix
# "Q" = Quantitative (numeric / continuous trait)
# "N" = Nominal (categorical trait)
...
traits_summ$"tr_types" # Summary of trait types
traits_summ$"mod_list"

####################################################@
# functional distance
sp_dist <- mFD::funct.dist( # computes a pairwise distance matrix among species based on their traits
  sp_tr         = traits_fd, # Species × traits data frame
  tr_cat        = traits_cat, # Trait types table (numeric vs categorical)
  metric        = "gower", # Gower distance, suitable for mixed data types
  scale_euclid  = "scale_center", # Standardizes quantitative traits (centering and scaling)
  ordinal_var   = "classic",
  weight_type   = "equal", # All traits weighted equally
  stop_if_NA    = F)
...
phy <- fishtree::fishtree_phylogeny(species = s_i)                                         # Requested 236 but only found 171 species, 74 not in tree (Phylo does not contain Elasmobranch)
